...
<head>
 ...
 <style>
  ...
  .btn-green { background-color: #8B4513; } /* brun foncé */
  .btn-red { background-color: #8B4513; }   /* remplace le rouge */
  .btn-yellow { background-color: #DEB887; } /* beige/brun clair */
  ...
  .device-item:hover { background-color: #dcd2ca; }
...
<body class="bg-gray-100">
  ...
  <header>
    <h1 class="text-center text-3xl font-bold mt-4" style="color: #F44336;">Bienvenue dans le Jardin !</h1>
  </header>
...
document.getElementById("device-cards").innerHTML = "";
    ...
    fetch('get_devices.php?salle=jardin')
  ...
  const options = Object.entries(deviceConfig)
    .filter(([key, config]) => config.default_room === "jardin")
    .map(([key]) => {
...
const data = { type, nom_salle: "Jardin" };
...
allFields.forEach(field => {
  ...
  fetch('add_device.php?salle=jardin', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
