...
console.log("Points JS =", userPoints);
  ...
  window.addEventListener('DOMContentLoaded', () => {
    Promise.all([
      fetch('device_config.json').then(res => res.json()),
      fetch('rules_config.json').then(res => res.json())
    ]).then(([config, rules]) => {
      deviceConfig = config;
      deviceRules = rules;

      // Injecter les infos PHP dans JS
      window.userRole = phpUserRole;
      window.userStatut = phpUserStatut;
      window.isReadOnly = phpUserStatut === 'visiteur';

	window.isReadOnly = userStatut === 'visiteur';
	window.isLimited = userStatut === 'simple';

      // Fonction pour tester les permissions d’un type d’objet
  ...
  return rules.roles.includes(userRole);
      ...
      loadDevices(); // Charger les appareils après toutes les infos
    }).catch(err => {
...
async function startTimer(cardId, type, table, idField, id) {
  ...
  // Vérifier les points nécessaires selon rules_config
  ...
  console.log(`Points requis pour ${duration}s (${type}) : ${pointsRequis}`);
  console.log(`Points utilisateur : ${window.userPoints}`);

  if (window.userPoints < pointsRequis) {
    alert("Vous n'avez pas assez de points pour démarrer cet appareil.");
    return;
  }
  ...
  try {
    ...
    if (!data.success) {
      alert("Erreur lors de la déduction des points : " + data.error);
      return;
    }

    window.userPoints = data.newPoints; // Met à jour les points côté client
    console.log("Points restants :", window.userPoints);

  } catch (err) {
    console.error("Erreur réseau :", err);
    alert("Erreur réseau lors de la déduction des points.");
    return;
  }
  ...
  const interval = setInterval(() => {
    ...
    if (remaining < 0) {
      ...
      // Éteindre fonctionnement automatiquement
      fetch('update_device.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
