...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$points = 0;
        ...
        if ($user) {
            // Mise à jour
            $stmt = $pdo->prepare("UPDATE utilisateur SET role = :role, statut = :statut, points, = :points, code_validation = :code, verifie = 0, demande_validee = 1 WHERE email = :email");
            $stmt->execute([
                ':role' => $role,
                ':statut' => $statut,
                ':code' => $code,
		...
		':points' => $points
            ...
            // Insertion
            $stmt = $pdo->prepare("INSERT INTO utilisateur (email, role, statut, code_validation, verifie, demande_validee, points) 
                                   VALUES (:email, :role, :statut, :code, 0, 1, :points)");
            $stmt->execute([
                ':email' => $email,
                ':role' => $role,
                ':statut' => $statut,
